...
class LpySimulation:
    ...
    def post_profile(self,task):
        ...
        drawProfileTable(task.profileView,task.profile_stats,rt,task.timing)
    def clear(self):
        self.lsystem.clear()
        self.setTree(None,0)
    def cancel(self):
...
def profileItem(st,rule_table=None,timing= None):
    ...
    if not strtype:
        sis += [MyItem(st.code.co_filename),MyItem(st.code.co_firstlineno)]
    for si in sis:
        si.setEditable(False)
    return sis
...
def drawProfileTable(table,stats,rule_table = None,timing = None):
        nstats = sort_stats(stats)
        optionModel = QStandardItemModel(0, 1)
        ...
        st_stack = []
        for st in nstats:
            items = profileItem(st,rule_table,timing)
            ...
            if st.calls :
              ...
              while len(st_stack) > 0:
                ...
                for sst in refst.calls:
                    subitems = profileItem(sst,rule_table,timing)
