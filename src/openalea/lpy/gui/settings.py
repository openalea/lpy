...
def restoreState(lpywidget):
  #try:
    ...
    lpywidget.displayMetaInfo = settings.value('displayMetaInfoAtRun',QVariant(lpywidget.displayMetaInfo)).toBool() 
    settings.endGroup()
    ...
    lc = settings.value('lpyshell',QVariant(True)).toBool()
    sc = settings.value('sysconsole',QVariant(False)).toBool()
...
def saveState(lpywidget):
    ...
    settings.setValue('displayMetaInfoAtRun',QVariant(lpywidget.displayMetaInfo))
    settings.endGroup()
    settings.beginGroup('compilation')
    ...
    if not lpywidget.interpreter is None:
        ...
        settings.setValue('lpyshell',QVariant(errinshell))
        settings.setValue('sysconsole',QVariant(errinsys))
        settings.endGroup()
    settings.beginGroup('syntax')
