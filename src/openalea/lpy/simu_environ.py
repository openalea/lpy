...
def scalarCode(scalars = None, indentation = '\t'):
    ...
    if not scalars is None and len(scalars) > 0:
        init_txt += indentation+'scalars = '+str([(i.name,i.value,i.minvalue,i.maxvalue) for i in scalars])+'\n'
        init_txt += indentation+'context["__scalars__"] = scalars\n'
        init_txt += indentation+'for n,v,mnv,mxv in scalars:\n'+indentation+'\tcontext[n] = v\n'
    return init_txt
