...
/*---------------------------------------------------------------------------*/
  ...
  ModuleClassTable::alias(const std::string &aliasname, const std::string &name)
  {
	...
	if (*italias == *itname)
	...
	else
	  {
	    if (itname->second->isActive())
	      {
		LsysError("Redeclaration of alias '" + aliasname + "' as '" + name + "' (previously '" +
			  italias->second->name + "').");
  ...
  ModuleClassTable::alias(const std::string &aliasname, ModuleClassPtr module)
  {
	...
	modulenamemap[aliasname] = module;
      ...
      {
	LsysError("Redeclaration of alias '" + aliasname + "' as '" + module->name + "' (previously '" +
		  italias->second->name + "').");
      }
  }
  ...
  ModuleClassTable::remove(const std::string &name)
  {
	...
	else if ((aliasiter = std::find(it->second->aliases.begin(), it->second->aliases.end(), name)) !=
