...
public:
	...
	static inline void append_modargs(element_type&  value, const std::vector<bp::object>& elements){
	   for(std::vector<bp::object>::const_iterator it = elements.begin(); it != elements.end(); ++it)
			value.append(*it);
	}
};
