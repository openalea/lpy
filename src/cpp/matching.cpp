...
/*---------------------------------------------------------------------------*/
  ...
  void MatchingEngine::setModuleMatchingMethod(MatchingEngine::eModuleMatchingMethod method)
  {
    ...
    switch (ModuleMatchingMethod)
      {
	case eMSimple:
	  ModuleMatchingFunc = &MatchingImplementation::simple_module_matching;
	break;
	case eMWithStar:
	  ModuleMatchingFunc = &MatchingImplementation::module_matching_with_star;
	break;
	default:
	  ModuleMatchingFunc = &MatchingImplementation::module_matching_with_star_and_valueconstraints;
	break;
      }
  }
  ...
  void MatchingEngine::setStringMatchingMethod(MatchingEngine::eStringMatchingMethod method)
  {
	...
	LeftMatchingFunc = &MatchingImplementation::string_left_match;
	break; /*StringReverseMatcher<GetPrevious>::match*/
	case eMScaleAxialTree:
	  RightMatchingFunc = &MatchingImplementation::mstree_right_match;
	LeftMatchingFunc = &MatchingImplementation::mstree_left_match;
	break;
	case eMLevelAxialTree:
	  RightMatchingFunc = &MatchingImplementation::mltree_right_match;
	LeftMatchingFunc = &MatchingImplementation::mltree_left_match;
	break;
	case eAxialTree:
	default:
	  RightMatchingFunc = &MatchingImplementation::tree_right_match;
	LeftMatchingFunc = &MatchingImplementation::tree_left_match;
	break;
      }
  }
...
/*---------------------------------------------------------------------------*/
				   ...
				   ArgList &params, AxialTree::IteratorMap *itermap)
  {
    return (*RightMatchingFunc)(matching_start, string_begin, string_end,
				pattern_begin, pattern_end,
				last_matched, matching_end, filter, params, itermap);
/*	switch(StringMatchingMethod){
		case eString:
  ...
  bool MatchingEngine::left_match(AxialTree::const_iterator matching_start,
				  ...
				  ArgList &params, AxialTree::IteratorMap *itermap)
  {
    return (*LeftMatchingFunc)(matching_start, string_begin, string_end, pattern_rbegin, pattern_rend, matching_end,
			       filter, params, itermap);
/*	switch(StringMatchingMethod){
		case eString:
							 ...
							 ArgList &l)
  {
	...
	if (patternsize == 1)
	...
	else
	  {
	    ...
	    if (patternsize == 2 && vlastisKwds && vbeforelastisArgs)
	      { // (**kwds,*args)
		...
		boost::python::dict largs;
		int startkwd = std::max<int>(0,
					     normalparam - std::max<int>(0, modulesize - module.getNamedParameterNb()));
	...
	if (patternsize == 0) return modulesize == 0;
	  ...
	  { // args >= 2
	    ...
	    if (patternsize == 2 && vbeforelastisArgs && vlastisKwds)
	      { // (*args,**kwds)
		size_t nbNamedParameter = module.getNamedParameterNb();
		if (nbNamedParameter > modulesize) return false;
		boost::python::dict largs;
	...
	if (patternsize == 1)
	  {
		...
		if (v.isNamed() || v.hasCondition())
		  { // if necessary we retrieve the args
	  ...
	  { // patternsize > 2
		...
		if (vlast.isNamed() || vlast.hasCondition())
		  {
		    int startkwd = std::max<int>(0, normalparam -
						    std::max<int>(0, modulesize - module.getNamedParameterNb()));
...
/*---------------------------------------------------------------------------*/
						  ...
						  ArgList &params)
  {
    ...
    match(matching_start, string_begin, string_end, pattern_begin, pattern_end, matching_end, last_matched, filter,
	  params);
  }
  ...
  bool MatchingImplementation::string_left_match(AxialTree::const_iterator matching_start,
						 ...
						 ArgList &params, AxialTree::IteratorMap *itermap)
  {
    matching_start = StringPrevious<AxialTree::const_iterator, PatternString::const_reverse_iterator, AxialTree::IteratorMap>::next(
	    matching_start, pattern_rbegin, string_begin, string_end, filter);
    bool res = StringReverseMatcher<>::
    match(matching_start, string_begin, string_end, pattern_rbegin, pattern_rend, matching_end, filter, params);
    if (res)
      matching_end = StringNext<AxialTree::const_iterator, PatternString::const_reverse_iterator, AxialTree::IteratorMap>::next(
	      matching_end, pattern_rbegin, string_end, filter);
    return res;
  }
						...
						ArgList &params, AxialTree::IteratorMap *itermap)
  {
    ...
    match(matching_start, string_beg, string_end, pattern_begin, pattern_end, last_matched, matching_end, filter,
	  params, itermap);
