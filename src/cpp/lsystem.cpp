...
Lsystem::LsysAcquirer::LsysAcquirer(const Lsystem *lsys) : __lsys(lsys)
{ lsys->acquire(); }

Lsystem::LsysAcquirer::~LsysAcquirer()
{ __lsys->release(); }
...
Lsystem::__addInterpretationRule(const std::string &code, size_t groupid, int lineno, const ConsiderFilterPtr filter)
{
  ...
  r.consider(filter);
  if (!r.isContextFree())
    LsysWarning("Interpretation rules should be context free. Contexts not supported for multiple iterations.");
...
void Lsystem::addSubLsystem(const Lsystem &sublsystem)
{
  ...
  size_t groupid = 0;
  for (std::vector<RuleGroup>::const_iterator itg = sublsystem.__rules.begin();
       itg != sublsystem.__rules.end(); ++itg, ++groupid)
    {
		   ...
		   _it2 != mruleset.end(); _it2++)
		{
		  ...
		  if ((*_it2)->match(workingstring, _it, targetstring, _it3, args, &itermap))
			...
			} catch (error_already_set)
			{
		      ...
		      if (match == 1)
			{
			  if (debugger.shouldStop(_it, _it3, *_it2))
			    debugger.total_match(_it, _it3, targetstring, prodlength, *_it2, args);
			  _it = _it3;
			  break;
			}
		      else if (match == 0 && debugger.shouldStop(_it, _it3, *_it2))
			debugger.partial_match(_it, _it3, targetstring, *_it2, args);
		    }
		}
	      if (match != 1)
		{
      ...
      while (_it != _end)
	{
	  ...
	  for (RulePtrSet::const_iterator _it2 = mruleset.begin();
	      ...
	      if ((*_it2)->reverse_match(workingstring, _it, targetstring, _it3, args, &itermap))
		{
		  ...
		  if (match)
		    {
		      if (debugger.shouldStop(_it3 == _end ? _beg : _it3 + 1, _it + 1, *_it2))
			debugger.total_match(_it3 == _end ? _beg : _it3 + 1, _it + 1, targetstring, prodlength, *_it2,
					     args);
		      _it = _it3;
		      break;
		    }
		  else if (debugger.shouldStop(_it3 == _end ? _beg : _it3 + 1, _it + 1, *_it2))
		    debugger.partial_match(_it3 == _end ? _beg : _it3 + 1, _it + 1, targetstring, *_it2, args);
		}
	    }
	  if (!match)
	    {
...
Lsystem::__step(AxialTree &workingstring,
		...
		eDirection direction)
{
  if (__context.multicoreProcessing())
  ...
  if (direction == eForward)
    {
      ...
      if (__context.brackectMappingOptimLevel() >= 1)
	{ itermap = &_itermap; }
      if (__context.brackectMappingOptimLevel() == 2)
  ...
  if (direction == eForward)
	...
	startmoduleid.push_back(threadid * nbsymbolperthread);
      ...
      QFuture<AxialTree> result = QtConcurrent::mappedReduced(startmoduleid,
							      boost::bind(partialForwardStep, _1, nbsymbolperthread,
									  workingstring, ruleset),
							      assemble, QtConcurrent::OrderedReduce |
	...
	startmoduleid.push_back(threadid * nbsymbolperthread);
      ...
      QFuture<AxialTree> result = QtConcurrent::mappedReduced(startmoduleid,
							      boost::bind(partialBackwardStep, _1, nbsymbolperthread,
									  workingstring, ruleset),
							      assemble, QtConcurrent::OrderedReduce |
									QtConcurrent::SequentialReduce);
  ...
  while (_it != _endit && !interpreter.earlyReturn())
      ...
      else
	{
	  ...
	  if (match)
	    {
	      if (maxdepth > 1)
		__gRecursiveInterpretation < Interpreter > (ltargetstring, ruleset, interpreter, maxdepth - 1, false);
	      else
		{
...
struct TurtleInterpreter
{
  ...
  static inline bool earlyReturn()
  { return false; }
...
struct TurtleStepInterpreter
{
  ...
  inline bool earlyReturn()
  { return context.isEarlyReturnEnabled(); }
  ...
  {
    turtle.setId(0);
    context.enableEarlyReturn(false);
  }
  ...
  if (!workstring.empty() && nb_iter > 0)
      ...
      if (!__rules.empty() || no_match_no_return)
	{
	  ...
	  for (; (matching || no_match_no_return) && i < nb_iter; ++i)
		...
		{
		  if (!hasDebugger())
		    workstring = __step(workstring, production, previouslyinterpreted ? false : productionHasQuery,
					matching, dir);
		  else
		    workstring = __debugStep(workstring, production, previouslyinterpreted ? false : productionHasQuery,
					     matching, dir, *__debugger);
