...
const std::vector<float> LpyParsing::getSupportedFormat()
{
  std::vector<float> res;
  res.push_back(LPY_FORMAT_VERSION);
  return res;
}

bool LpyParsing::isSupportedFormat(float format)
{ return (1.0f <= format && format <= LPY_FORMAT_VERSION); }
...
inline
void ToEndlineCheckColon(std::string::const_iterator &_it, std::string::const_iterator _end, std::string &filename,
			 int &lineno)
{
  bool colon = false;
  while (_it != _end && (*_it) != '\n')
...
{
  while (_it != _end && (*_it) != '\n' && (*_it) != 'A' && (*_it) != 'I' && (*_it) != 'a' && (*_it) != 'm' &&
	 (*_it) != 'n' && (*_it) != 'p' && (*_it) != '#' && (*_it) != '\'' && (*_it) != '"')
    ++_it;
  if (_it != _end && (*_it) == '\n')
    {
      ++lineno;
...
float LpyParsing::getFormatVersion(std::string::const_iterator &it, std::string::const_iterator begcode,
				   std::string::const_iterator endcode)
{
  ...
  if (has_pattern(_it, begcode, endcode, std::string(VersionTag.begin(), VersionTag.begin() + p)))
	...
	{ ++_it; }
      ...
      if (has_pattern(_it, begcode, endcode, std::string(VersionTag.begin() + p + 2, VersionTag.end())) &&
	  (_it != endcode && *_it == '\n'))
	{
      ...
      switch (mode)
	{
	  case -1:
	      ...
	      switch (*_it)
		{
		  ...
		  case 'A':
		    _it2 = _it;
		  if ((has_keyword_pattern(_it, begcode, endpycode, "Axiom") ||
		       has_keyword_pattern(_it, begcode, endpycode, "axiom")) &&
			...
			LsysParserSyntaxError("Cannot find ':' after Axiom");
		      beg = _it;
		    }
		  else
		    {
		      ...
		      if (!modules.second.empty())
			{
			  ...
			  if (it != modules.second.end())
			      ...
			      if (!inherit.empty())
				{
				  ...
				  std::vector<std::string> inheritedclassnames = extract_vec<std::string>(
					  __context.evaluate(inherit))();
				  for (std::vector<std::string>::const_iterator iticl = inheritedclassnames.begin();
				       iticl != inheritedclassnames.end(); ++iticl)
				    {
			   ...
			   itmod != modules.first.end(); ++itmod)
			{
			  ...
			  if (!itmod->alias)
			      ...
			      if (!itmod->parameters.empty())
				{
				  ...
				  for (std::vector<std::string>::const_iterator itarg = args.begin();
				       itarg != args.end(); ++itarg)
				    {
				      if (!LpyParsing::isValidVariableName(*itarg))
					LsysError("Invalid parameter name '" + *itarg + "'", "", lineno);
				    }
		      ...
		      if (_it != endpycode && (*_it) == ':')
			{
			  ...
			  if (beg != endpycode)
				...
				max_derivation_lineno = lineno - 1;
			      ...
			      LsysParserSyntaxError("Cannot find value for derivation length");
			  else
			    LsysParserSyntaxError("Cannot find value for derivation length");
			}
		      else
			LsysParserSyntaxError("Cannot find ':' after derivation length");
		      beg = _it;
		    }
		  else
		    toendlineA(_it, endpycode);
		  ...
		  if (has_keyword_pattern(_it, begcode, endpycode, "lpyimport"))
		      ...
		      if (_it != endpycode)
			{
			  ...
			  if (beg != endpycode)
				...
				LsysParserSyntaxError("invalid module to import");
			    }
			  else
			    LsysParserSyntaxError("invalid module to import");
			}
		  ...
		  if (has_keyword_pattern(_it, begcode, endpycode, "consider"))
		      ...
		      if (_it != endpycode && (*_it) == ':')
			{
			  ...
			  if (beg != endpycode)
			    ...
			    LsysParserSyntaxError("Cannot find value for consider");
			}
		      else
			LsysParserSyntaxError("Cannot find ':' after consider");
		      code += "# " + std::string(_it2, _it);
		      beg = _it;
		    }
		  else
		  ...
		  if (has_keyword_pattern(_it, begcode, endpycode, "ignore"))
		      ...
		      if (_it != endpycode && (*_it) == ':')
			{
			  ...
			  if (beg != endpycode)
			    ...
			    LsysParserSyntaxError("Cannot find value for ignore");
			}
		      else
			LsysParserSyntaxError("Cannot find ':' after ignore");
		      code += "# " + std::string(_it2, _it);
		      beg = _it;
		    }
		  else
		  ...
		  case 'I': // python comments before rules
		    _it2 = _it;
		  if (has_keyword_pattern(_it, begcode, endpycode, "InLeftContext") ||
		      has_keyword_pattern(_it, begcode, endpycode, "InRightContext"))
		    {
		      code += std::string(beg, _it2);
		      while (_it != endpycode && (*_it == ' ' || *_it == '\t'))++_it;
		      ...
		      if (epit == _it)
			{
		      ...
		      code += std::string("p") + std::string(_it2, _it + 1) + TOOLS(number)(pid) +
			      std::string(epit, eit);
		      beg = eit;
		    }
		  else
	  ...
	  case 2:
		...
		if (mode == 0 && has_keyword_pattern(_it, begcode, endpycode, "derivation length"))
		...
		else if (has_keyword_pattern(_it, begcode, endpycode, "def"))
		  {
		    ...
		    beg = _it2;
		    LsysParserWarning(
			    "a python function is declared inside production section. Switching to python mode.");
		  }
		else
		  {
		    beg = _it;
		...
		if (has_keyword_pattern(_it, begcode, endpycode, "group"))
		  {
			...
			if (notOnlySpace(beg, _it))
			  ...
			  LsysParserSyntaxError("Cannot find value for group");
			toendline(_it, endpycode);
			code += "# " + std::string(_it2, _it);
		      }
		    else
		      LsysParserSyntaxError("Cannot find value and ':' after group");
		  }
		else
		  {
		    beg = _it;
			...
			if (beg != endpycode)
			  {
				...
				code += var + " = " + std::string(beg, _it);
			      ...
			      LsysParserSyntaxError("Cannot find value for maximum depth");
			  }
			else
			  LsysParserSyntaxError("Cannot find value for maximum depth");
		      }
		    else
		      LsysParserSyntaxError("Cannot find ':' after maximum depth");
		  }
		else
		  {
		    beg = _it;
	  ...
	  if (t.check())
	    {
	      ...
	      LsysWarning("Floating point value is given for derivation Length. Rounding.", filename,
			  max_derivation_lineno);
	    }
	  ...
	  if (t.check())
	    {
	      ...
	      LsysWarning("Floating point value is given for decomposition maximum length. Rounding.", filename,
			  decomposition_max_depth_lineno);
	    }
	  else
	    LsysError("Decomposition maximum length has an invalid value.", filename, decomposition_max_depth_lineno);
	}
    }
	  ...
	  if (t.check())
	    {
	      ...
	      LsysWarning("Floating point value is given for homomorphism maximum length. Rounding.", filename,
			  homomorphism_max_depth_lineno);
	    }
...
void LsysRule::set(const std::string &rule)
{
  ...
  while (endheader != rule.end() && !foundendheader)
      ...
      if (*endheader == ':')
	{
	  ...
	  if (distance(marker, rule.end()) >= staticmarkertxt.size() &&
	      std::string(marker, marker + staticmarkertxt.size()) == staticmarkertxt)
	    {
      ...
      else if (*endheader == '-' && (endheader == rule.begin() || *(endheader - 1) != '-'))
	{
	  ...
	  else if (distance(endheader, rule.end()) >= staticarrowtxt.size() &&
		   std::string(endheader, endheader + staticarrowtxt.size()) == staticarrowtxt)
	    {
	      foundendheader = true;
	      arrow = true;
	      staticrule = true;
      ...
      else if (*_it == 'n' && std::distance<std::string::const_iterator>(_it, _end) > 8)
	{
	  if (std::string(_it, _it + 8) == "nproduce")
	      ...
	      if (*_it == '(')
		{
	      ...
	      definition += "pproduce";
	      definition += LpyParsing::lstring2pyparam(_it, _end, endproduction,
							lineno == -1 ? lineno : lineno + llineno);
	      if (endproduction == ')') ++_it;
	      _lastit = _it;
	    }
      ...
      else if (has_keyword_pattern(_it, _beg, _end, "InLeftContext") ||
	       has_keyword_pattern(_it, _beg, _end, "InRightContext"))
	{
	  definition.insert(definition.end(), _lastit, _cit);
	  while (_it != _end && (*_it == ' ' || *_it == '\t'))++_it;

	  if (*_it != '(')
	    LsysError("Cannot find opening bracket of InContext", "", lineno == -1 ? lineno : lineno + llineno);
...
LsysRule::parseHeader(const std::string &header)
{
  ...
  while (it != end)
      ...
      else if (*it == '>')
	{
	  ...
	  if (*(it + 1) == '>')
	    {
	      if (begncd || !ncd.empty())
		LsysError("Ill-formed Rule Header : Two new left contexts found : " + header, "", lineno);
	      else
		{
  ...
  if (!ncg.empty() && !ncd.empty())
    LsysError("Ill-formed Rule Header : New left and right contexts found : " + header, "", lineno);
	  ...
	  if (LsysContext::current()->warnWithSharpModule())
	    {
	      ...
	      if (_it != endpos && *_it == '(')
		LsysSyntaxError(
			"Found symbol '#' after Lstring. Considered as begining of comments in Lpy (Compatibility pb with cpfg). Use '_' instead.",
			"", lineno);
			     ...
			     char delim)
{
  ...
  while (_it != endpos && *_it != delim)
	...
	{
	  if (*_it != ',')
	    LsysSyntaxError(std::string("Invalid syntax in module declaration: Wait for ','instead of '") + *_it + "'");
	  ++_it;
	}
      while (_it != endpos && (*_it == ' ' || *_it == '\t'))++_it;
      if (_it == endpos || *_it == delim || *_it == '\n' || *_it == ':' || *_it == '#') break;
	...
	{ result.push_back(ModDeclaration(name)); }
      while (_it != endpos && (*_it == ' ' || *_it == '\t'))++_it;
      if (_it == endpos) break;
      if (*_it != ',' && *_it != '\n' && *_it != ':' && *_it != '=' && *_it != '(' && *_it != '#')
