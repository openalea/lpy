   ...
   public:
      ...
      if (it2->isRepExp())
	{
	  ...
	  while (ok && numiter < maxiter)
	    {
	      argtype lp;
	      if ((ok = Matcher::match(it, string_beg, string_end, lpattern.begin(), lpattern.end(), last_matched, it,
				       filter, lp, iteratormap)))
		{
		  llp.push_back(lp);
		  last_matched = it;
		  ++it;
	  ...
	  for (int ip = 0; ip < it2->argSize(); ++ip)
	    {
	      ...
	      if (matched == -1)
		{
		  if (Matcher::match(it, string_beg, string_end, lpattern.begin(), lpattern.end(), last_matched, it,
				     filter, lp, iteratormap))
		    {
		      matched = ip;
		    }
    ...
    static bool reverse_match(Iterator matching_start, Iterator string_begin, Iterator string_end,
			      PIterator pattern, Iterator &matching_end, const ConsiderFilterPtr &filter,
			      argtype &lparams, IteratorMap *iteratormap = NULL)
    {
      Iterator it = matching_start;
      PIterator it2 = pattern;
      if (it2->isRepExp())
	{
	  ...
	  if (numiter == 0)
	    ...
	    { ArgsCollector::prepend_args(lparams, ArgsCollector::fusion_args(llp)); }
	}
      else if (it2->isOr())
	{
	  int matched = -1;
...
/*---------------------------------------------------------------------------*/
  ...
  struct StringMatcher
  {
      ...
      for (PIterator it2 = pattern_begin; it2 != pattern_end; ++it2)
	{
	  ...
	  if (it2->isGetModule())
	    { if (!process_get_module(it2, it, string_beg, string_end, lp, filter)) return false; }
	  else if (it2->isRE())
	    {
	      if (!RegExpMatcher<MType>::match(it, string_beg, string_end, it2, pit, it, filter, lp, iteratormap))
		return false;
	    }
	  else
	    {
...
/*---------------------------------------------------------------------------*/
  ...
  struct StringReverseMatcher
  {
      ...
      for (PIterator it2 = pattern_rbegin; it2 != pattern_rend;)
	{
	  if (it2->isRE())
	    {
	      if (!RegExpMatcher<MType>::reverse_match(it, string_begin, string_end, it2, it, filter, lp,
						       iteratormap))
		return false;
	      ++it2;
	    }
	  else
	    {
	      argtype lmp;
	      if (it2->isGetModule())
		{ if (!process_get_module(it2, it, string_begin, string_end, lmp, filter)) return false; }
	  ...
	  if (it2->isStar())
	      ...
	      if (it2->isRightBracket())
		{
		  if (!it->isRightBracket())
			...
			{
			  bool found = false;
			  Iterator begbracket = bracketstack.top();
			  bracketstack.pop();
			  if (iteratormap)
			    {
			      typename IteratorMap::const_iterator res = iteratormap->find(
				      iter_to_hashable(begbracket, string_end));
