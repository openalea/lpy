...
LsysContext::done()
{
  ...
  else if (this != DEFAULT_LSYSCONTEXT)
    { LsysError("Not current context trying to be done."); }
}

bool
LsysContext::isCurrent() const
{
  if (!CURRENT_LSYSCONTEXT)
    { CURRENT_LSYSCONTEXT = GLOBAL_LSYSCONTEXT; }
...
void LsysContext::init_options()
{
  ...
  option->addValue("On the fly", &ModuleClassTable::setMandatoryDeclaration, false,
		   "Module declaration is made on the fly when parsing string.");
  option->addValue("Mandatory", &ModuleClassTable::setMandatoryDeclaration, true,
		   "Module declaration is mandatory before use in string.");
  option->setDefault(0);
  option->setGlobal(true);
  /** Warning with sharp module option */
  option = options.add("Warning with sharp module",
		       "Set whether a warning is made when sharp symbol is met when parsing (compatibility with cpfg).",
		       "Compilation");
  ...
  option->addValue("Simple", &MatchingEngine::setModuleMatchingMethod, MatchingEngine::eMSimple,
		   "Simple module matching : Same name and same number of arguments . '*' module allowed.");
  option->addValue("Extension : *args", &MatchingEngine::setModuleMatchingMethod, MatchingEngine::eMWithStar,
		   "Add matching rule that * module can match any module and allow *args to match a number of module arguments");
  option->addValue("Extensions : *args and arg value constraints", &MatchingEngine::setModuleMatchingMethod,
		   MatchingEngine::eMWithStarNValueConstraint,
		   "With * module and *args. A module argument can also be set to a given value adding thus a new matching constraint.");
  option->setDefault(MatchingEngine::eDefaultModuleMatching);
  option->setGlobal(true);
  /** module matching option */
  option = options.add("Module inheritance",
		       "Specify if modules inheritance is taken into account in rules pattern matching", "Matching");
  option->addValue("Disabled", &MatchingEngine::setInheritanceModuleMatchingActivated, false,
		   "Modules with different names are considered as different even if they inherit from each other.");
  option->addValue("Enabled", &MatchingEngine::setInheritanceModuleMatchingActivated, true,
		   "A module inheriting from a second one can be used for application of the first one if number of parameters are also compatible.");
  option->setDefault(0);
  option->setGlobal(true);
  /** string matching option */
  option = options.add("String matching", "Specify the way strings are matched to rules pattern", "Matching");
  option->addValue("As String", &MatchingEngine::setStringMatchingMethod, MatchingEngine::eString,
		   "String is considered as a simple string.");
  option->addValue("As AxialTree", &MatchingEngine::setStringMatchingMethod, MatchingEngine::eAxialTree,
		   "String is considered as an axial tree and some modules can be skipped according to tree connectivity.");
  option->addValue("As Multi-level AxialTree", &MatchingEngine::setStringMatchingMethod,
		   MatchingEngine::eMLevelAxialTree,
		   "String is considered as a multi level axial tree and some modules can be skipped according to module level. Level are not ordered.");
  option->addValue("As Multiscale AxialTree", &MatchingEngine::setStringMatchingMethod,
		   MatchingEngine::eMScaleAxialTree,
		   "String is considered as a multi scale axial tree and some modules can be skipped according to module scale.");
  option->setDefault(MatchingEngine::eDefaultStringMatching);
  option->setGlobal(true);
  /** early return when no matching option */
  option = options.add("Early return when no matching",
		       "Set whether the L-systems end prematurely if no matching has occured in the last iteration.",
		       "Processing");
  ...
  option = options.add("Warning with Turtle inconsistency",
		       "Set whether a warning/error is raised when an invalid value is found during turtle processing.",
		       "Processing");
  ...
  option = options.add("Bracket mapping optimization",
		       "Specify the level of optimization of mapping brackets for lstring traversal.", "Processing");
  option->addValue("Disabled", this, &LsysContext::setBracketMappingOptimLevel, 0,
		   "Disable brackets mapping optimization.");
  option->addValue("On the fly", this, &LsysContext::setBracketMappingOptimLevel, 1,
		   "Enable brackets mapping on the fly.");
  option->addValue("As preprocessing", this, &LsysContext::setBracketMappingOptimLevel, 2,
		   "Compute brackets mapping as preprocessing of each step.");
  option->setDefault(0);

  /** selection required option */
  option = options.add("Selection Always Required",
		       "Set whether selection check in GUI is required or not. Selection is then transform in X module in the Lstring.",
		       "Interaction");
...
inline bool is_a_null_ptr(LstringMatcher *lm)
{ return lm == NULL; }

bool LsysContext::pInLeftContext(size_t pid, boost::python::dict &res)
{
  if (!__lstringmatcher.hasLocalData() || is_a_null_ptr(*(__lstringmatcher.localData())))
    LsysError("Cannot call InLeftContext");
...
{
  if (!__lstringmatcher.hasLocalData() || is_a_null_ptr(*(__lstringmatcher.localData())))
    LsysError("Cannot call InRightContext");
