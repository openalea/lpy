...
#define IterMap(Iterator) pgl_hash_map<size_t, Iterator>
  ...
  size_t iter_to_hashable(Iterator iter, Iterator string_end)
  { return (size_t) &(*iter); /*std::distance(iter, string_end);*/ }
  ...
  Iterator endBracket(Iterator pos, Iterator string_end, IteratorMap *itermap = NULL, bool startingBeforePos = false)
  {
	...
	if (mappedend != itermap->end())
	  {
	    Iterator lpos = pos;
	    if (mappedend->second == string_end)
	      { pos = mappedend->second; }
	    else
	      { pos = mappedend->second + 1; }
	...
	if (pos->isLeftBracket())
	...
	else if (pos->isRightBracket())
	  {
	    assert(bracket > 0);
	    bracket--;
	    Iterator mappedbegin = iterstack.top();
	    iterstack.pop();
  ...
  template<class Iterator, class IteratorMap = IterMap(Iterator) >
  Iterator beginBracket(Iterator pos, Iterator string_begin, Iterator string_end, IteratorMap *itermap = NULL,
			bool startingAfterPos = false)
  {
	...
	if (pos->isRightBracket())
	  {
	    ...
	    if (mappedbegin != itermap->end())
	      {
		if (mappedbegin->second == string_begin)
		  { pos = mappedbegin->second; }
		else
		  { pos = mappedbegin->second - 1; }
	      }
	    else
	      {
		iterstack.push(pos);
  ...
  template<class Iterator, class IteratorMap = IterMap(Iterator) >
  Iterator
  parent(Iterator pos, Iterator string_begin, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(),
  ...
  template<class Iterator, class IteratorMap = IterMap(Iterator) >
  std::vector<Iterator>
  children(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(),
	   IteratorMap *itermap = NULL)
  {
  ...
  template<class Iterator, class IteratorMap  = IterMap(Iterator) >
  std::vector<Iterator>
  lateral_children(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(),
		   IteratorMap *itermap = NULL)
  {
    std::vector<Iterator> result;
    // current pos is the end of a branch
  ...
  template<class Iterator, class IteratorMap = IterMap(Iterator) >
  std::vector<Iterator>
  roots(Iterator string_begin, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(),
	IteratorMap *itermap = NULL)
  {
  ...
  Iterator complex(Iterator pos, int scale, Iterator string_begin, Iterator string_end,
		   const ConsiderFilterPtr filter = ConsiderFilterPtr(), IteratorMap *itermap = NULL)
  {
    if (pos == string_begin) return string_end;
  ...
  template<class Iterator, class IteratorMap = IterMap(Iterator) >
  std::vector<Iterator>
  components(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(),
	     IteratorMap *itermap = NULL)
  {
    std::vector<Iterator> result;
    // current pos is the end of a branch
...
*/
				...
				const ConsiderFilterPtr filter = ConsiderFilterPtr(), IteratorMap *itermap = NULL)
  {
    ...
    int curscale = pos->scale();
    if (!is_lower_scale(targetscale, previousscale))
      { // mean that we look for a predecessor, not a complex
	// Go up into complex.
...
*/
			      ...
			      IteratorMap *itermap = NULL)
  {
    ...
    if (!is_upper_scale(scale, previous_scale))
      { // mean that we look for a successor, not a components
	// Skip successor components
    ...
    // If exists, we are on the child or at least one of its complex.
    int curscale = pos->scale();
    if (is_lower_scale(curscale, scale))
      { // We look for a component of current module
	// Go down into components.
	do
	  {
	    pos = direct_child(pos, string_end, filter, itermap);
		       ...
		       const ConsiderFilterPtr filter = ConsiderFilterPtr())
  {
    ...
    while (pos != string_end && pos->isIgnored(filter))
      { ++pos; }
    return pos;
  }
			   ...
			   const ConsiderFilterPtr filter = ConsiderFilterPtr())
  {
    ...
    while (pos != string_begin && pos->isIgnored(filter))
      { --pos; }
    return pos;
  }
