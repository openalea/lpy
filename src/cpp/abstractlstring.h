...
/*---------------------------------------------------------------------------*/
  ...
  class AbstractLString
  {
   ...
   protected:
    ...
    void resetString()
    { __data = AbstractLStringInternalPtr(new AbstractLStringInternal()); }
   ...
   public:
    ...
    { return __conststring().empty(); }

    inline size_t size() const
    { return __conststring().size(); }

    inline size_t capacity()
    { return size(); /*__conststring().capacity();*/ }
      ...
      while (!isEnd(_it))
	{
	  if (eq(i, _it))
	    {
	      *_it = j[0];
	      insertAt(_it + 1, j.const_begin() + 1, j.const_end());
	    }
	  ++_it;
	}
    }
    ...
    children(const_iterator pos, const ConsiderFilterPtr filter = ConsiderFilterPtr()) const
    { return LPY::children(pos, const_end(), filter); }

    inline std::vector<const_iterator>
    lateral_children(const_iterator pos, const ConsiderFilterPtr filter = ConsiderFilterPtr()) const
    ...
    complex(const_iterator pos, int scale, const ConsiderFilterPtr filter = ConsiderFilterPtr()) const
    ...
    inline std::vector<const_iterator>
    components(const_iterator pos, const ConsiderFilterPtr filter = ConsiderFilterPtr()) const
    { return LPY::components(pos, const_end(), filter); }

    inline std::vector<const_iterator>
    components_at_scale(const_iterator pos, int scale, const ConsiderFilterPtr filter = ConsiderFilterPtr()) const
    ...
    inline bool operator!=(const AbstractLStringType &other) const
    ...
    inline const Module &first() const
    { return __conststring().front(); }

    inline Module &first()
    { return __string().front(); }

    inline const Module &last() const
    { return __conststring().back(); }

    inline Module &last()
    { return __string().back(); }
