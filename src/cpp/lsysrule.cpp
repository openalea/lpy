...
std::string LsysRule::callerFunctionName() const
{ return functionName() + "_caller"; }
...
void LsysRule::importPyFunction()
{
  if (!isCompiled())
    {
      __function = LsysContext::currentContext()->getObject(
	      __nbParams <= MAX_LRULE_DIRECT_ARITY ? functionName() : callerFunctionName());
...
boost::python::object LsysRule::__call_function(size_t nbargs, const ArgList &args) const
{
  switch (nbargs)
	...
	return __function();
      ...
      case 1:
	return __function(args[0]);
      break;
      case 2:
	return __function(args[0], args[1]);
      break;
      case 3:
	return __function(args[0], args[1], args[2]);
      break;
      case 4:
	return __function(args[0], args[1], args[2], args[3]);
      break;
      case 5:
	return __function(args[0], args[1], args[2], args[3], args[4]);
      break;
      case 6:
	return __function(args[0], args[1], args[2], args[3], args[4], args[5]);
      break;
      case 7:
	return __function(args[0], args[1], args[2], args[3], args[4], args[5], args[6]);
      break;
      case 8:
	return __function(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
      break;
      case 9:
	return __function(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8]);
      break;
      case 10:
	return __function(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9]);
      break;
      case 11:
	return __function(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9],
			  args[10]);
...
#endif
      default:
	return __function(toPyList(args));
      break;
    }
}
...
template<class T>
inline void extend_vec(T &v, const T &v2)
  ...
  while (_first != _last)
    {
	  ...
	  != __formalparameters.end())
	return distance(__formalparameters.begin(), _first);
      _first++;
    };
  return -1;
}
		...
		eDirection direction) const
{
  ...
  if (direction == eForward)
    {
      if (!MatchingEngine::match(pos, src.const_begin(), src.const_end(), __predecessor.const_begin(),
				 __predecessor.const_end(), endpos1, last_match, __consider, args_pred))
	{
	  return false;
	}
    }
  ...
  if (direction == eBackward && !__newrightcontext.empty())
    {
      ...
      if (!MatchingEngine::right_match(dest.const_begin(), dest.const_begin(), dest.const_end(),
				       ...
				       endposNewRightLastMatch, endposNewRight, __consider, args_ncd /*, itermap*/))
	return false;
  ...
  if (!__rightcontext.empty())
    {
      ...
      if (!MatchingEngine::right_match(endposRight, src.const_begin(), src.const_end(),
				       ...
				       endposRightLastMatch, endposRight, __consider, args_cd, itermap))
	return false;
	...
	__map(ModuleClass::getMaxId()), __nbrules(rules.size()), __maxsmb(0)
{
  ...
  for (RulePtrSet::const_iterator it = rules.begin(); it != rules.end(); ++it)
    {
      std::vector<size_t> ids = (direction == eForward ? (*it)->predecessor().getFirstClassId()
						       : (*it)->predecessor().getLastClassId());
      for (std::vector<size_t>::const_iterator itid = ids.begin(); itid != ids.end(); ++itid)
	{
	  ...
	  if (*itid == ModuleClass::Star->getId())
	      ...
	      if (MatchingEngine::isInheritanceModuleMatchingActivated() && !derivedclasses.empty())
		{
		  ...
		  for (ModuleClassList::const_iterator itCl = derivedclasses.begin();
		       itCl != derivedclasses.end(); ++itCl)
