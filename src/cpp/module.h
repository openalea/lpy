...
/*---------------------------------------------------------------------------*/
  ...
  class LPY_API Module
  {
   public:
    ...
    inline void setName(const std::string &c)
    { __mclass = ModuleClassTable::get().getClass(c); }

    inline bool sameName(const Module &m) const
    { return __mclass == m.__mclass; }

    inline ModuleClassPtr getClass() const
    {
      lpyassert(__mclass != NULL);
      return __mclass;
    }
    ...
    inline bool operator==(const Module &n) const
    { return sameName(n); }
    ...
    { return __mclass == ModuleClass::None; }
    ...
    inline bool isRepExp() const
    { return __mclass == ModuleClass::RepExp; }

    inline bool isOr() const
    { return __mclass == ModuleClass::Or; }

    inline bool isRE() const
    { return isRepExp() || isOr(); }

    inline bool isGetIterator() const
    { return __mclass == ModuleClass::GetIterator; }

    inline bool isGetModule() const
    ...
    { return __mclass->getScale(); }

   protected:
    inline void setClass(size_t cid)
    { __mclass = ModuleClassTable::get().find(cid); }
...
/*---------------------------------------------------------------------------*/
  ...
  class AbstractParamModule : public Module
  {
   public:
    ...
    { return __constargs().empty(); }

    /*attribute_deprecated*/ inline size_t argSize() const
    { return __constargs().size(); }

    /*attribute_deprecated*/ inline bool hasArg() const
    { return !__constargs().empty(); }
    ...
    {
      size_t ri, rj;
      getValidIndices(i, j, ri, rj);
      return getSlice(ri, rj);
    }
    ...
    inline void delSliceItemAt(int i, int j)
    {
      size_t ri, rj;
      getValidIndices(i, j, ri, rj);
      return delSlice(ri, rj);
    }

    inline void append(const ParameterType &value)
    { __args().push_back(value); }

    inline void prepend(const ParameterType &value)
    { __args().insert(__args().begin(), value); }
   ...
   protected:
    inline size_t getValidParameterPosition(const std::string &pname) const
    {
      ...
      if (pos >= size())
	LsysError("Inexisting value for parameter '" + pname + " '(" + TOOLS(number)(pos) + ") for module '" + name() +
		  "'.");
      return pos;
    }
    ...
    inline ParameterList &__args()
    { return __argholder->__args; }

    inline const ParameterList &__args() const
    { return __argholder->__args; }

    inline const ParameterList &__constargs() const
    { return __argholder->__args; }
    ...
    static inline ParameterList toParameterList(const boost::python::object &t)
    {
      ...
      try
	{ while (1) result.push_back(bp::extract<ParameterType>(iter_obj.attr("next")())()); }
...
/*---------------------------------------------------------------------------*/
  ...
  class LPY_API ParamModule : public AbstractParamModule<boost::python::object>
  {
   public:
    ...
    template<class T>
    T _get(int i) const
    { return boost::python::extract<T>(getAt(i))(); }

    template<class T>
    bool _check(int i) const
    { return boost::python::extract<T>(getAt(i)).check(); }


    virtual void _setValues(real_t, real_t, real_t);

    inline void _setValues(const TOOLS(Vector3) &v)
    { _setValues(v.x(), v.y(), v.z()); }

    virtual void
    _setFrameValues(const TOOLS(Vector3) &p, const TOOLS(Vector3) &h, const TOOLS(Vector3) &u, const TOOLS(Vector3) &l);
    ...
    inline void interpret(PGL::Turtle &t)
    { getClass()->interpret(*this, t); }
...
/*---------------------------------------------------------------------------*/
  ...
  inline bool is_lower_scale(int scale1, int scale2)
  { return scale1 < scale2; }

  inline bool is_eq_scale(int scale1, int scale2)
  { return scale1 == scale2; }

  inline bool is_neq_scale(int scale1, int scale2)
  { return scale1 != scale2; }

  inline bool is_upper_scale(int scale1, int scale2)
  { return scale1 > scale2; }

  inline int up_scale(int scale1)
  { return scale1 + 1; }

  inline int down_scale(int scale1)
  { return scale1 - 1; }
