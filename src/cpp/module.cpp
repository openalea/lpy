...
boost::python::object LPY::getFunctionRepr()
{ return GlobalContext::getFunctionRepr(); }
...
void
processArgList(ModuleClassPtr mclass, ParamModule::ParameterList &args, boost::python::object arg, size_t start = 0)
{
  ...
  if (!isdict.check())
    {
      ...
      try
	{ while (true) appendParam(args, iter_obj.attr("next")()); }
      ...
      if (nbstdparam + len(arg) < mclass->getNamedParameterNb())
	{
	  ...
	  str << mclass->name << " takes exactly " << mclass->getNamedParameterNb() << " (" << nbstdparam + len(arg)
	      << " given)";
	  LsysError(str.str());
	}
	...
	{
	  std::stringstream str;
	  str << mclass->name << " takes exactly " << mclass->getNamedParameterNb() << " (" << missingargs.size()
	      << " missing)";
	  LsysError(str.str());
	}
    }
			 ...
			 boost::python::object arg, size_t start = 0)
{
  ...
  if (l > 0)
      ...
      for (size_t i = start; i < l - 1; ++i)
	{
      ...
      if (l > start)
	{ processLastArg(module.getClass(), args, arg[l - 1]); }
    }
}
...
ParamModule::ParamModule(const std::string &name,
	...
	BaseType(name)
{ processLastArg(getClass(), __args(), a); }
...
ParamModule::ParamModule(const std::string &name,
	...
	BaseType(name)
{
  appendParam(__args(), a);
  processLastArg(getClass(), __args(), b);
}
...
{
  appendParam(__args(), a);
  appendParam(__args(), b);
  processLastArg(getClass(), __args(), c);
}
	...
	BaseType(name)
{
  appendParam(__args(), a);
  appendParam(__args(), b);
  appendParam(__args(), c);
  processLastArg(getClass(), __args(), d);
}
